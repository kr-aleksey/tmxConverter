...
def convert_tmx_to_excel(tmx_path, output_path):
    ...
    for tu in tu_elements:
        ...
        for tuv in tu.findall("tuv"):
            lang = tuv.attrib.get("{http://www.w3.org/XML/1998/namespace}lang", "unknown")
            seg = tuv.find("seg")
            if seg is not None:
...
def main():
    ...
    button = tk.Button(root, text="Выбрать файл", command=select_and_convert_file)
    button.pack()
